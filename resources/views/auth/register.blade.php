...
<x-app-layout>
    ...
    @if (session('status'))
        <div
            class="mb-4 font-medium text-sm text-green-600 p-4 bg-green-100 border border-green-400 rounded-md"
        >
            {{ session('status') }}
        </div>
    @endif
    ...
    <x-guest-layout>
        <form
            method="POST"
            action="{{ auth()->check() ? route('register.petugas.store') : route('register') }}"
        >
            @csrf
            ...
            <div class="mt-4" id="kelas_wrapper">
                ...
                >
                    ...
                    @foreach (['PPLG', 'APHP', 'AKL', 'TO', 'APAT'] as $jurusan)
                ...
                @endphp
                            @for ($i = 1; $i <= $range; $i++)
                                @php $val = "X " . $jurusan . " " . $i; @endphp
                                <option
                                    value="{{ $val }}"
                                    {{ old('kelas') == $val ? 'selected' : '' }}
                                >
                                    {{ $val }}
                                </option>
                            @endfor
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            kelasSelect = new TomSelect('#kelas', {
                create: false,
                sortField: {
                    field: 'text',
                    direction: 'asc',
                ...
                render: {
                    optgroup_header: function (data, escape) {
                        return (
                            '<div class="optgroup-header font-bold text-indigo-600 p-2 bg-gray-100">' +
                            escape(data.label) +
                            '</div>'
                        );
                    },
