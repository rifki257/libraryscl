...
>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <x-slot name="trigger">
            <button class="relative inline-flex items-center p-2 rounded-full text-gray-600 hover:text-black hover:bg-gray-100 transition-all focus:outline-none">
        ...
        <x-slot name="content">
                        ...
                        <div class="px-4 py-3 hover:bg-gray-50 transition border-b border-gray-50 last:border-0 {{ $notification->read_at ? 'opacity-60' : '' }}">
                            <div class="flex flex-col gap-1">
                                <div class="flex justify-between items-start">
                                    <span class="text-[10px] font-bold text-red-600 uppercase">Peminjaman Ditolak</span>
                                ...
                                @if(isset($notification->data['alasan']))
                                    <p class="text-[11px] text-gray-500 italic bg-gray-100 p-1.5 rounded mt-1">
                                        "{{ $notification->data['alasan'] }}"
                                    </p>
                                @endif
                            </div>
                        </div>
            ...
            @auth
                @if(auth()->user()->notifications->count() > 0)
                    <a href="{{ route('markNotificationsRead') }}" class="block w-full text-center py-2 text-[11px] font-bold text-indigo-600 hover:bg-indigo-50 border-t border-gray-100">
                        Tandai Semua Dibaca
                    </a>
                @endif
            @endauth
    ...
    {{-- akhir notif --}}
                            <a
                                {{-- Logika Toggle: Jika aktif di wishlist, klik lagi balik ke dashboard/katalog --}}
